...
class Model(object):
  def __init__(self,
    ...
    with tf.device("/cpu:0"):
      ...
      def _pre_process(x):
          x = tf.nn.embedding_lookup(dic, x)
          return x

      self.x_train = tf.map_fn(_pre_process, x_train, dtype=tf.float32, back_prop=False)
      self.y_train = y_train

      # valid data
      self.x_valid, self.y_valid = None, None
      if text["valid"] is not None:
        text["valid_original"] = np.copy(text["valid"])
        labels["valid_original"] = np.copy(labels["valid"])
        self.num_valid_examples = np.shape(text["valid"])[0]
        self.num_valid_batches = (
          (self.num_valid_examples + self.eval_batch_size - 1)
          // self.eval_batch_size)
        x_valid, y_valid = tf.train.batch(
          [text["valid"], labels["valid"]],
        ...
        self.x_valid = tf.map_fn(_pre_process, x_valid, dtype=tf.float32, back_prop=False)
        self.y_valid = y_valid

      self.num_test_examples = np.shape(text["test"])[0]
      self.num_test_batches = (
        (self.num_test_examples + self.eval_batch_size - 1)
        // self.eval_batch_size)
      x_test, y_test = tf.train.batch(
        [text["test"], labels["test"]],
        batch_size=self.eval_batch_size,
        capacity=10000,
        enqueue_many=True,
        num_threads=1,
      ...
      self.x_test = tf.map_fn(_pre_process, x_test, dtype=tf.float32, back_prop=False)
      self.y_test = y_test

    # cache text and labels
    self.text = text
    self.labels = labels
