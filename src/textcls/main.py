...
from src.textcls.data_utils import read_data
from src.textcls.general_controller import GeneralController
from src.textcls.general_child import GeneralChild

from src.textcls.micro_controller import MicroController
from src.textcls.micro_child import MicroChild
...
def get_ops(text, labels, dic):
  """
  Args:
    text: dict with keys {"train", "valid", "test"}.
...
def train():
  if FLAGS.child_fixed_arc is None:
    text, labels, dic = read_data(FLAGS.data_path)
  else:
    text, labels, dic = read_data(FLAGS.data_path, num_valids=0)

  g = tf.Graph()
  with g.as_default():
    ops = get_ops(text, labels, dic)
    child_ops = ops["child"]
