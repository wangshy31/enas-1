...
from src.textcls.models import Model
from src.textcls.image_ops import conv
from src.textcls.image_ops import fully_connected
from src.textcls.image_ops import batch_norm
from src.textcls.image_ops import batch_norm_with_mask
from src.textcls.image_ops import relu
from src.textcls.image_ops import max_pool
from src.textcls.image_ops import drop_path
from src.textcls.image_ops import global_avg_pool
...
class MicroChild(Model):
  ...
  def _model(self, text, is_training, reuse=False):
    """Compute the logits given the text."""
    ...
    with tf.variable_scope(self.name, reuse=reuse):
      ...
      with tf.variable_scope("stem_conv"):
        w = create_weight("w", [1, 300, 1, self.out_filters * 3])
        x = tf.nn.conv2d(
          text, w, [1, 1, 1, 1], "VALID", data_format=self.data_format)
        x = batch_norm(x, is_training, data_format=self.data_format)

        w_2 = create_weight("w_2", [1, 1, self.out_filters * 3, self.out_filters * 3])
        x = tf.nn.conv2d(
          x, w_2, [1, 1, 1, 1], "VALID", data_format=self.data_format)
        x = batch_norm(x, is_training, name='bn2', data_format=self.data_format)

        w_3 = create_weight("w_3", [1, 1, self.out_filters * 3, self.out_filters * 3])
        x = tf.nn.conv2d(
          x, w_3, [1, 1, 1, 1], "VALID", data_format=self.data_format)
        x = batch_norm(x, is_training, name='bn3', data_format=self.data_format)
        ...
        if (self.use_aux_heads and
          ...
          with tf.variable_scope("aux_head"):
            ...
            with tf.variable_scope("avg_pool"):
              ...
              w = create_weight("w", [hw, 1, inp_c, 128])
              aux_logits = tf.nn.conv2d(aux_logits, w, [1, 1, 1, 1], "SAME",
                                        data_format=self.data_format)
              aux_logits = batch_norm(aux_logits, is_training=True,
                                      data_format=self.data_format)
  ...
  def _fixed_conv(self, x, f_size, out_filters, stride, is_training,
    ...
    for conv_id in range(stack_convs):
      ...
      with tf.variable_scope("sep_conv_{}".format(conv_id)):
        w_depthwise = create_weight("w_depth", [f_size, 1, inp_c, 1])
  ...
  def _enas_cell(self, x, curr_cell, prev_cell, op_id, out_filters):
    ...
    with tf.variable_scope("avg_pool"):
      avg_pool = tf.layers.average_pooling2d(
        x, [3, 1], [1, 1], "SAME", data_format=self.actual_data_format)
      ...
      for conv_id in range(stack_conv):
        with tf.variable_scope("stack_{0}".format(conv_id)):
          ...
          w_depthwise = create_weight(
            "w_depth", [num_possible_inputs, filter_size * 1 * inp_c])
          w_depthwise = w_depthwise[prev_cell, :]
          w_depthwise = tf.reshape(
            w_depthwise, [filter_size, 1, inp_c, 1])
  ...
  def build_valid_rl(self, shuffle=False):
    ...
    with tf.device("/cpu:0"):
      # shuffled valid data: for choosing validation model
        ...
        #self.text["valid_original"] = np.transpose(
          #self.text["valid_original"], [0, 3, 1, 2])
      x_valid_shuffle, y_valid_shuffle = tf.train.shuffle_batch(
        [self.text["valid_original"], self.labels["valid_original"]],
      ...
      def _pre_process(x):
          x = tf.nn.embedding_lookup(self.dic, x)
          return x

      #if shuffle:
      x_valid_shuffle = tf.map_fn(_pre_process, x_valid_shuffle, dtype=tf.float32, back_prop=False)
    ...
    print (valid_shuffle_preds.shape)
    print (y_valid_shuffle.shape)
